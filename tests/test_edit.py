...
class TestEdit(BaseTest, AutobuildBaselineCompare):
    ...
    def test_platform_configure_ios(self):
        ...
        args = [
            ...
            "--config-file=%s" % self.tmp_file,
            "name=darwin_ios",
            "build_directory=foo/bar/baz_ios",
        ]
        built_config = self._try_cmd(args)
        self.assertEqual(
            built_config["package_description"]["platforms"]["common"][
                "build_directory"
            ],
            "foo/bar/baz",
        )
        self.assertEqual(
            built_config["package_description"]["platforms"]["darwin_ios"][
                "build_directory"
            ],
            "foo/bar/baz_ios",
        )
