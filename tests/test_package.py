...
$""" % (
            ...
            "1.0",
            re.escape(
                os.path.join(self.data_dir, "package-test", "autobuild-package.xml")
            ),
            "common",
            re.escape(self.tar_name),
            "[0-9a-f]{32}",
            "[0-9a-f]{128}",
            "[0-9a-f]{40}",
            "[0-9a-f]{64}",
        ...
        assert os.path.exists(results_output), (
            "results file not found: %s" % results_output
        )
    ...
    def test_results_json(self):
        ...
        with open(results_file) as f:
            ...
            self.assertEqual(
                results["autobuild_package_metadata"],
                os.path.join(self.data_dir, "package-test", "autobuild-package.xml"),
            )
    ...
    def test_package_other_version(self):
        ...
        metadata_filename = os.path.join(
            build_directory, configfile.PACKAGE_METADATA_FILE
        )
        ...
        if not os.path.exists(expected_tar_name):
            if os.path.exists(self.tar_name):
                raise AssertionError(
                    "package built %s instead of %s"
                    % (self.tar_name, expected_tar_name)
                )
            raise AssertionError(
                "package built neither %s nor %s" % (self.tar_name, expected_tar_name)
            )

    def test_autobuild_package(self):
        with CaptureStdout():
            self.autobuild(
                "package", "--config-file=" + self.config_path, "-p", "common"
            )
        assert os.path.exists(self.tar_name), "%s does not exist" % self.tar_name
        self.tar_has_expected(self.tar_name)
        ...
        assert os.path.exists(self.tar_zst_name), (
            "%s does not exist" % self.tar_zst_name
        )
        ...
        self.autobuild(
            "package", "--config-file=" + self.config_path, "-p", "common", "--dry-run"
        )
        assert not os.path.exists(self.zip_name), (
            "%s created by dry run" % self.zip_name
        )
        assert not os.path.exists(self.tar_name), (
            "%s created by dry run" % self.tar_name
        )
    ...
    def test_disallowed_paths(self):
        ...
        self.instantiateTemplate(
            config_template, platform_config, {"PLATFORM": self.platform}
        )
