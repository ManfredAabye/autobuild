...
class TestConfigure(BaseTest, AutobuildBaselineCompare):
    ...
    def test_configure(self):
        ...
        result = configure._configure_a_configuration(
    ...
    def test_substitutions(self):
        ...
        assert "foo32" in self.autobuild(
            "configure", "--config-file=" + self.tmp_file, "-A", "32"
        )
        assert "foo64" in self.autobuild(
            "configure", "--config-file=" + self.tmp_file, "-A", "64"
        )
    ...
    def test_id(self):
        ...
        assert "foo666" in self.autobuild(
            "configure", "--config-file=" + self.tmp_file, "-i", "666"
        )
