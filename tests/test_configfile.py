...
class TestConfigFile(BaseTest, AutobuildBaselineCompare):
    ...
    def test_configuration_simple(self):
        ...
        assert (
            reloaded.package_description.platforms["common"]
...
class TestExpandVars(BaseTest):
    ...
    def test_expand_vars_string(self):
        ...
        self.assertEqual(
            configfile._expand_vars_string("no subs", self.vars), "no subs"
        )
        # simple substitutions handled by string.Template
        self.assertEqual(
            configfile._expand_vars_string("'$one', '${two}'", self.vars), "'1', '2'"
        )
        ...
        self.assertEqual(
            configfile._expand_vars_string("abc${one|fallback}def", self.vars),
            "abc1def",
        )

        # undefined expands to fallback
        self.assertEqual(
            configfile._expand_vars_string("abc${four|fallback}def", self.vars),
            "abcfallbackdef",
        )
        ...
        self.assertEqual(
            configfile._expand_vars_string(
                "abc_${one|nope}_def_${four|nofour}_ghi_${five|}_jkl", self.vars
            ),
