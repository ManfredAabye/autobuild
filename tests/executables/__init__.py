...
def echo(text):
    return Executable(
        ...
        options=[os.path.join(_SCRIPT_DIR, "echo.py")],
        arguments=[text],
    )
...
noop = Executable(
    command=sys.executable, options=[os.path.join(_SCRIPT_DIR, "noop.py")]
)
envtest = Executable(
    command=sys.executable, options=[os.path.join(_SCRIPT_DIR, "envtest.py")]
)
