...
def query_manifest(options=None):
    ...
    return dict(
        (item.get("package_description").get("name"), item) for item in sequence
    )
    ...
    class FakeOptions(object):
        ...
        def copy(self):
            ...
            return self.__class__(
                **dict((attr, getattr(self, attr)) for attr in self._attrs)
            )
...
class BaseTest(TestCase):
    ...
    def setup_method(self, module):
        ...
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "argparse-1.1-common-111.tar.bz2"), SERVER_DIR
        )
    ...
    def localizedConfig(self, template):
        ...
        temp_config_fd, config_filename = tempfile.mkstemp(
            prefix=os.path.join(mydir, "data", temp_config_basename + "-"),
            suffix="-local.xml",
        )
        temp_config = os.fdopen(temp_config_fd, "w")
        template_path = os.path.join(mydir, "data", template)
        logger.debug(
            "localize config file '%s' -> '%s'" % (template_path, config_filename)
        )
...
class TestInstallArchive(BaseTest):
    ...
    def test_update(self):
        ...
        assert os.path.exists(
            os.path.join(self.options.select_dir, "include", "bogus.h")
        )
        # Get ready to update with newer version of same package name
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "bogus-0.2-common-222.tar.bz2"), SERVER_DIR
        )
        self.options = FakeOptions(
    ...
    def test_update_move(self):
        ...
        assert_in(
            "0.2",
            open(os.path.join(self.options.select_dir, "include", "bogus.h")).read(),
        )

    def test_unknown(self):
        with ExpectError(
            "unknown package:", "Expected InstallError for unknown package name"
        ):
    ...
    def test_no_platform(self):
        self.options = FakeOptions(
            install_filename=self.localizedConfig("packages-failures.xml"),
            package=["noplatform"],
        )
        autobuild_tool_install.AutobuildTool().run(self.options)
        assert not os.path.exists(os.path.join(INSTALL_DIR, "lib", "bogus.lib"))
    ...
    def test_no_archive(self):
        ...
        with ExpectError(
            "noarchive", "Expected InstallError for missing ArchiveDescription"
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
    ...
    def test_no_url(self):
        ...
        with ExpectError(
            "no url specified", "Expected InstallError for missing archive url"
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
    ...
    def test_no_license(self):
        ...
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "nolicense-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        self.options = FakeOptions(
            install_filename=self.localizedConfig("packages-failures.xml"),
            package=["nolicense"],
        ...
        with ExpectError(
            "no license specified", "Expected InstallError for missing license"
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
    ...
    def test_no_metadata(self):
        ...
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "nometa-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        self.options = FakeOptions(
            install_filename=self.localizedConfig("packages-failures.xml"),
            package=["nometa"],
    ...
    def test_conflicting_file(self):
        ...
        autobuild_tool_install.AutobuildTool().run(self.options)
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "conflict-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        ...
        self.options.local_archives = [
            os.path.join(mydir, "data", "conflict-0.1-common-111.tar.bz2")
        ]
        with ExpectError(
            "attempts to install files already installed",
            "Expected InstallError for conflicting files",
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
    ...
    def test_conflicting_direct_depends(self):
        ...
        # first, install the default 'bogus' package (should succeed)
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "bingo-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        self.server_tarball = self.copyto(
            os.path.join(mydir, "data", "bongo-0.1-common-111.tar.bz2"), SERVER_DIR
        )
        # set up a new configuration file that installs 'bingo'
        self.options = FakeOptions(
            install_filename=self.localizedConfig("package-conflict.xml"),
            package=["bingo"],
        )
        autobuild_tool_install.AutobuildTool().run(self.options)
        ...
        with ExpectError(
            "not installable due to conflicts",
            "Expected InstallError for dependency conflicts",
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
        # then try to install the 'bongo' package locally (should fail)
        self.options.local_archives = [
            os.path.join(mydir, "data", "bongo-0.1-common-111.tar.bz2")
        ]
        with ExpectError(
            "not installable due to conflicts",
            "Expected InstallError for dependency conflicts",
    ...
    def test_conflicting_indirect_depends(self):
        ...
        # then try to install the 'bongo' package locally (should fail)
        self.options.local_archives = [
            os.path.join(mydir, "data", "bingo-0.1-common-111.tar.bz2")
        ]
        with ExpectError(
            "not installable due to conflicts",
            "Expected InstallError for dependency conflicts",
    ...
    def test_copyrights(self):
        ...
        self.assertEqual(
            stream.getvalue(),
            "Copyright 2014 Linden Research, Inc.\nbogus: The Owner\n",
        )
...
class TestInstallCachedBZ2Archive(BaseTest):
    def setup_method(self, method):
        ...
        self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.tar.bz2"),
...
class TestInstallCachedGZArchive(BaseTest):
    def setup_method(self, method):
        ...
        self.cache_name = in_dir(
            common.get_install_cache_dir(), "bogus-0.1-common-111.tar.gz"
        )

        # Instead copy directly to cache dir.
        self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.tar.gz"),
            common.get_install_cache_dir(),
        )
...
class TestInstallCachedXZArchive(BaseTest):
    def setup_method(self, method):
        ...
        self.cache_name = in_dir(
            common.get_install_cache_dir(), "bogus-0.1-common-111.tar.xz"
        )

        # Instead copy directly to cache dir.
        self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.tar.xz"),
            common.get_install_cache_dir(),
        )
...
class TestInstallCachedZSTArchive(BaseTest):
    def setup_method(self, method):
        ...
        self.cache_name = in_dir(
            common.get_install_cache_dir(), "bogus-0.1-common-111.tar.zst"
        )

        # Instead copy directly to cache dir.
        self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.tar.zst"),
            common.get_install_cache_dir(),
        )
...
class TestInstallCachedZIPArchive(BaseTest):
    def setup_method(self, method):
        ...
        self.cache_name = in_dir(
            common.get_install_cache_dir(), "bogus-0.1-common-111.zip"
        )

        # Instead copy directly to cache dir.
        self.copyto(
            os.path.join(mydir, "data", "bogus-0.1-common-111.zip"),
            common.get_install_cache_dir(),
        )
...
class TestInstallLocalArchive(BaseTest):
    def setup_method(self, method):
        ...
        clean_file(
            in_dir(common.get_install_cache_dir(), "bogus-0.1-common-111.tar.bz2")
        )
    ...
    def test_only_local(self):
        ...
        # should not happen
        assert not os.path.exists(
            os.path.join(self.options.select_dir, "LICENSES", "argparse.txt")
        )
...
class TestGarbledDownload(BaseTest):
    ...
    def test_bad(self):
        ...
        with ExpectError(
            "Failed to download", "expected InstallError for md5 mismatch"
        ):
            autobuild_tool_install.AutobuildTool().run(self.options)
        assert not os.path.exists(self.cache_name)
...
class TestInstall(BaseTest):
    def test_install_all(self):
        ...
        assert os.path.exists(
            os.path.join(self.options.select_dir, "LICENSES", "bogus.txt")
        )
        assert os.path.exists(os.path.join(self.options.select_dir, "lib", "bogus.lib"))
        assert os.path.exists(
            os.path.join(self.options.select_dir, "include", "bogus.h")
        ...
        assert os.path.exists(
            os.path.join(self.options.select_dir, "lib", "python2.5", "argparse.py")
        )
...
class TestDownloadPackage(unittest.TestCase):
    ...
    def test_download_github(self, mock_urlopen: MagicMock):
        ...
        with envvar("AUTOBUILD_GITHUB_TOKEN", "token-123"):
            autobuild_tool_install.download_package(
                "https://example.org/foo.tar.bz2", creds="github"
            )
            mock_urlopen.assert_called()
            _, args, _ = mock_urlopen.mock_calls[0]
            got_req = args[0]
            ...
            self.assertEqual(
                got_req.unredirected_hdrs["Accept"], "application/octet-stream"
            )
    ...
    def test_download_gitlab(self, mock_urlopen: MagicMock):
        ...
        with envvar("AUTOBUILD_GITLAB_TOKEN", "token-123"):
            autobuild_tool_install.download_package(
                "https://example.org/foo.tar.bz2", creds="gitlab"
            )
            mock_urlopen.assert_called()
            _, args, _ = mock_urlopen.mock_calls[0]
            got_req = args[0]
