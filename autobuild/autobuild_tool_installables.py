...
class AutobuildTool(autobuild_base.AutobuildBase):
    ...
    def register(self, parser):
        ...
        parser.add_argument(
            "name", nargs="?", default=None, help="the name of the installable"
        )
        parser.add_argument(
            "argument", nargs="*", help="a key=value pair specifying an attribute"
        )
...
def _get_new_metadata(config, args_name, args_archive, arguments):
    ...
    if args_archive and "url" in key_values:
        raise InstallablesError(
            "--archive (%s) and url (%s) may not both be specified"
    ...
    for description_key in _PACKAGE_ATTRIBUTES:
        if description_key in key_values:
            ...
            if (
                description_key in metadata.package_description
                and metadata.package_description[description_key] is not None
                and key_values[description_key]
                != metadata.package_description[description_key]
            ):
                raise InstallablesError(
                    "command line %s (%s) does not match archive %s (%s)"
                    % (
                        description_key,
                        key_values[description_key],
                        description_key,
                        metadata.package_description[description_key],
                    )
                )
    ...
    for archive_key in _ARCHIVE_ATTRIBUTES:
        if archive_key in key_values:
            if (
                archive_key in metadata.archive
                ...
                raise InstallablesError(
                    ...
                    % (
                        archive_key,
                        key_values[archive_key],
                        archive_key,
                        metadata.archive[archive_key],
                    )
                )
            else:
    ...
    if "platform" in key_values:
        if (
            ...
            raise InstallablesError(
                "specified platform '%s' does not match archive platform '%s'"
                % (key_values["platform"], metadata["platform"])
            )
        else:
            platform = key_values.pop("platform")
    else:
        if "platform" in metadata and metadata["platform"] is not None:
...
def edit(config, args_name, args_archive, arguments):
    ...
    if args_name and args_name != package_name:
        raise InstallablesError(
            "name argument (%s) does not match package name (%s)"
            % (args_name, package_name)
        )

    installed_package_description = config.installables[package_name]
    for element in _PACKAGE_ATTRIBUTES:
        if (
            element in metadata.package_description
            and metadata.package_description[element] is not None
        ):
            installed_package_description[element] = metadata.package_description[
                element
            ]
    ...
    if platform_name in installed_package_description.platforms:
        ...
        installed_package_description.platforms[platform_name] = (
            platform_description.copy()
        )
    ...
    for element in _ARCHIVE_ATTRIBUTES:
        if element in metadata.archive:
            if metadata.archive[element] is None:
                ...
                del installed_package_description.platforms[platform_name].archive[
                    element
                ]
            else:
                installed_package_description.platforms[platform_name].archive[
                    element
                ] = metadata.archive[element]
...
def _check_name(arg_name, key_values, metadata):
    ...
    if arg_name is not None:
        ...
        if (
            ...
            raise InstallablesError(
                "key/value name (%s) does not match archive name (%s)"
