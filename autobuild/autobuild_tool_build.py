...
class AutobuildTool(autobuild_base.AutobuildBase):
    def get_details(self):
        return dict(
            name=self.name_from_file(__file__), description="Builds platform targets."
        )
    ...
    def register(self, parser):
        ...
        parser.add_argument(
            "--id", "-i", dest="build_id", type=int, help="unique build number"
        )
    ...
    def run(self, args):
        ...
        try:
            ...
            for build_configuration in build_configurations:
                ...
                if result != 0:
                    raise BuildError(
                        "building configuration %s returned %d"
                ...
                metadata_file.package_description = configfile.PackageDescription(
                    bconfig.package_description
                )
                ...
                if config.package_description.use_scm_version:
                    try:
                        metadata_file.package_description.version = (
                            metadata_file.package_description.read_scm_version(
                                build_directory
                            )
                        )
                    except LookupError:
                ...
                metadata_file.package_description.platforms = (
                    None  # omit data on platform configurations
                )
                ...
                if common.is_env_enabled("AUTOBUILD_VCS_INFO"):
                    ...
                    if git:
                        metadata_file.package_description.vcs_branch = os.environ.get(
                            "AUTOBUILD_VCS_BRANCH", git.branch
                        )
                        metadata_file.package_description.vcs_revision = os.environ.get(
                            "AUTOBUILD_VCS_REVISION", git.revision
                        )
                        metadata_file.package_description.vcs_url = os.environ.get(
                            "AUTOBUILD_VCS_URL", git.url
                        )
                    else:
                        logger.warning(
                            "Unable to initialize git. Repository not found or git CLI not available"
                        )
                ...
                if args.select_dir:
                    ...
                    logger.debug("specified metadata directory: {}".format(install_dir))
                else:
                    ...
                    logger.debug(
                        "metadata in build subdirectory: {}".format(install_dir)
                    )
...
def _build_a_configuration(
    ...
    try:
        common_build_configuration = config.get_build_configuration(
            build_configuration.name, platform_name=common.PLATFORM_COMMON
        )
