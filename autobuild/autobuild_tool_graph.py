...
class AutobuildTool(autobuild_base.AutobuildBase):
    ...
    def register(self, parser):
        ...
        parser.add_argument(
            "source_file", nargs="?", default=None, help="package or metadata file."
        )
        parser.add_argument(
            "--config-file",
    ...
    def run(self, args):
        ...
        if not args.source_file:
            ...
            metadata_file = os.path.join(
                config.get_build_directory(args.configuration, platform),
            ...
            if not os.path.exists(metadata_file):
                ...
                install_dirs = common.select_directories(
                    ...
                    lambda cnf: os.path.join(
                        config.get_build_directory(cnf, platform), "packages"
                    ),
                )
                installed_pathname = os.path.join(
                    os.path.realpath(install_dirs[0]), args.installed_filename
                )
            ...
            logger.info(
                "searching for metadata in autobuild package metadata file %s"
                % args.source_file
            )
        ...
        if args.graph_type == "mermaid":
            ...
            def add_depends(sb: StringIO, pkg, root=False):
                ...
                version = (
                    pkg["package_description"]["version"]
                    if pkg["package_description"]["version"]
                    else ""
                )
            ...
            graph = pydot.Dot(
                label=metadata["package_description"]["name"]
                + incomplete
                + " dependencies for "
                + platform,
                graph_type="digraph",
            ...
            def add_depends(graph, pkg):
                ...
                try:
                    pkg_node = (
                        got if got is None or isinstance(got, pydot.Node) else got[0]
                    )
                except (
                    IndexError
                ):  # some versions of pydot may return an empty list instead of None
                    pkg_node = None
                if pkg_node is None:
                    ...
                    if "dirty" in pkg and (
                        pkg["dirty"] == "True" or pkg["dirty"] is True
                    ):
                        logger.debug(
                            " setting %s dirty: %s"
                    ...
                    if "dependencies" in pkg:
                        for dep_pkg in pkg["dependencies"].values():
                            ...
                            print(
                                f"[DEBUG] Adding dependency from {dep_node.get_name()} to {dep_name}"
                            )
                            logger.debug(
                                " graph adding dependency %s -> %s" % (dep_name, name)
                            )
                            edge = pydot.Edge(dep_name, name)
                            if "dirty" in dep_pkg and (
                                dep_pkg["dirty"] == "True" or dep_pkg["dirty"] is True
                            ):
            ...
            if args.display or args.graph_file:
                if args.graph_file:
                    ...
                    graph_file = os.path.join(
                        tempfile.gettempdir(),
                        metadata["package_description"]["name"]
                        + "_graph_"
                        + args.graph_type
                        + ".png",
                    )
