...
def handle_query_args(options, config_file, installed_file):
    ...
    if options.copyrights:
            ...
            if "dependencies" in packages:
                for pkg in packages["dependencies"].keys():
                    ...
                    if pkg not in copyrights:
                        if (
                            "copyright"
                            in packages["dependencies"][pkg]["package_description"]
                        ):
                            copyrights[pkg] = (
                                packages["dependencies"][pkg]["package_description"][
                                    "copyright"
                                ].strip()
                                + "\n"
                            )
        ...
        if (
            "package_description" in config_file
            and config_file.package_description.get("copyright") is not None
        ):
            all_copyrights = (
                "%s" % config_file.package_description["copyright"].strip() + "\n"
            )
        else:
    ...
    if options.list_dirty:
        ...
        dirty_pkgs = [
            installed[package]["package_description"]["name"]
...
def download_package(
    package_url: str, timeout=120, creds=None, package_name=""
...
def get_package_file(
    package_name, package_url, hash_algorithm="md5", expected_hash=None, creds=None
):
    """
    ...
    while cache_file is None and download_retries > 0:
        ...
        if os.path.exists(cache_file):
            ...
            if os.path.getsize(cache_file) == 0:
                ...
                cache_file = None
            elif hash_algorithm is not None and not verify_hash(
                hash_algorithm, cache_file, expected_hash
            ):
                logger.error(
                    "corrupt cached file removed: %s mismatch"
            ...
            logger.info(
                "downloading %s:\n  %s\n     to %s"
                % (package_name, package_url, cache_file)
            )
            try:
                package_response = download_package(
                    package_url,
            ...
            if package_response is not None:
                with open(cache_file, "wb") as cache:
                    max_block_size = (
                        1024 * 1024
                    )  # if this is changed, also change 'MB' in progress message below
                    package_size = int(
                        package_response.headers.get("content-length", 0)
                    )
                    package_blocks = (
                        package_size / max_block_size if package_size else 0
                    )
                    if package_blocks < (package_size * max_block_size):
                        package_blocks += 1
                    logger.debug(
                        "response size %d blocks %d" % (package_size, package_blocks)
                    )
                    blocks_recvd = 0
                            ...
                            if package_blocks:
                                print(
                                    ...
                                    % (
                                        blocks_recvd,
                                        package_blocks,
                                        int(100 * blocks_recvd / package_blocks),
                                    ),
                                    end=" ",
                                    flush=True,
                                )
...
def do_install(
    packages, config_file, installed, platform, install_dir, dry_run, local_archives=[]
):
    """
    ...
    for pname in packages:
        ...
        if pname in local_archives:
            ...
            if _install_binary(
                pname, platform, package, config_file, install_dir, installed, dry_run
            ):
                installed_pkgs.append(pname)
...
def _install_local(
    ...
    if metadata:
        ...
        _update_installed_package_files(
            metadata,
            installed_package,
            platform=platform,
            installed=installed,
...
def _install_binary(
    configured_name, platform, package, config_file, install_dir, installed, dry_run
):
    ...
    if not req_plat:
        logger.warning(
            "package %s has no installation information configured for platform %s"
    ...
    if not archive:
        raise InstallError(
            "no archive specified for package %s for platform %s"
            % (package_name, platform)
        )
    if not archive.url:
        raise InstallError(
    ...
    if installed_pkg and installed_pkg["install_type"] == "local":
        logger.warning(
            """skipping %s package because it was installed locally from %s
  To allow new installation, run
  autobuild uninstall %s"""
            % (package_name, installed_pkg["archive"]["url"], package_name)
        )
        return False
    ...
    cachefile = get_package_file(
        package_name,
        archive.url,
        hash_algorithm=(archive.hash_algorithm or "md5"),
        expected_hash=archive.hash,
        creds=(archive.creds or None),
    )
    if cachefile is None:
        raise InstallError(
            "Failed to download package '%s' from '%s'" % (package_name, archive.url)
        )
...
def _default_metadata_for_package(package_file: str, package=None):
    ...
    if package is not None:
        if from_name[0] != package.name:
            raise InstallError(
                "configured package name '%s' does not match name from archive '%s'"
...
def extract_package(
    package_file: str, install_dir: str, dry_run: bool = False
) -> ExtractPackageResults:
    with archive_utils.open_archive(package_file) as archive:
        ...
        for t in archive:
            if t.name == configfile.PACKAGE_METADATA_FILE:
                ...
                if (
                    os.path.exists(t_path)
                    and not os.path.isdir(t_path)
                    and t.name not in results.files
                ):
...
def _install_common(
    configured_name: str,
    platform: str,
    package: configfile.PackageDescription,
    package_file: str,
    install_dir: str,
    ...
    if installed_pkg:
        if verify_hash(
            installed_pkg["archive"].get("hash_algorithm", "md5"),
            package_file,
            installed_pkg["archive"]["hash"],
        ):
            logger.info("%s is already installed" % package.name)
            return None, None
        else:
            logger.info(
                f"{package.name} hash changed from {installed_pkg['archive']['hash']} to ''"
            )
            if not dry_run:
    ...
    try:
        ...
        if configured_name != package.name:
            raise InstallError(
                "Configured package name '%s' does not match name in package '%s'"
        ...
        if dependancy_conflicts:
            raise InstallError(
                """Package not installable due to conflicts
%s
  configuration %s
  version       %s
  build_id      %s
Conflict: %s
  ...
  try uninstalling those packages and rerunning."""
        ...
        if not (metadata.package_description.get("license") or package.license):
            raise InstallError(
                "no license specified in metadata or configuration for %s."
                % package.name
            )

        license_file = (
            metadata.package_description.get("license_file") or package.license_file
        )
        if license_file is None or not (
            license_file in extract_results.files
            or license_file.startswith("http://")
            or license_file.startswith("https://")
        ):
            raise InstallError(
                "nonexistent license_file for %s: %s" % (package.name, license_file)
            )
    except Exception:
...
def transitive_dependency_conflicts(new_package, installed):
    ...
    conflicts = ""
    logger.debug(
        "  checking conflicts for %s in installed"
    ...
    if "dependencies" in new_package:
        logger.debug(
            "  checking conflicts for dependencies of %s in installed"
            % new_package["package_description"]["name"]
        )
        ...
        for used in previous.keys():
            ...
            if new_package["package_description"]["name"] == used:
                if "archive" in new_package and new_package["archive"]:
                    ...
                    if (
                        new_package["archive"]["url"].rsplit("/", 1)[-1]
...
def _update_installed_package_files(
    metadata, package, platform=None, installed=None, install_dir=None, files=None
):
...
def uninstall(package_name, installed_config):
    ...
    logger.info(
        "uninstalling %s version %s"
        % (package_name, package.package_description.version)
    )
    clean_files(
        os.path.join(common.get_current_build_dir(), package.install_dir),
        package.manifest,
    )
...
def install_packages(args, config_file, install_dir, platform, packages):
    ...
    for archive_path in args.local_archives:
        ...
        if local_metadata.package_description.name in packages:
            ...
            raise InstallError(
                "no package '%s' found for local archive '%s'"
...
class AutobuildTool(autobuild_base.AutobuildBase):
    ...
    def register(self, parser):
        ...
        parser.add_argument(
            "package", nargs="*", help="List of packages to consider for installation."
        )
        parser.add_argument(
            "--config-file",
    ...
    def run(self, args):
        ...
        for build_configuration in build_configurations:
            ...
            install_dirs = common.select_directories(
                ...
                lambda cnf: os.path.join(
                    bconfig.make_build_directory(
                        cnf, platform=platform, dry_run=args.dry_run
                    ),
                    "packages",
                ),
            )
