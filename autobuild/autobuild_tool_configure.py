...
class AutobuildTool(autobuild_base.AutobuildBase):
    def get_details(self):
        return dict(name="configure", description="Configures platform targets.")
    ...
    def run(self, args):
        ...
        if package_errors:
            raise ConfigurationError(
                "%s\n    in configuration %s" % (package_errors, args.config_file)
            )
...
def _configure_a_configuration(
    config: configfile.BuildConfigurationDescription,
    build_configuration,
    extra_arguments,
    dry_run=False,
    environment=None,
